...
class _AnalyticsScreenState extends ConsumerState<AnalyticsScreen> {
  ...
  void _shareAnalytics(BuildContext context, AnalyticsState state) {
    final sb = StringBuffer();
    sb.writeln('📊 My AttendX Attendance Report');
    sb.writeln('─────────────────────────────');
    sb.writeln('Overall Rate : ${(state.overallRate ?? 0).toInt()}%');
    sb.writeln('Total Sessions: ${state.totalSessions}');
    sb.writeln(
        'Present: ${state.totalPresent}  Late: ${state.totalLate}  Absent: ${state.totalAbsent}');
    sb.writeln('Current Streak: ${state.currentStreak} sessions');
    sb.writeln('Best Streak   : ${state.longestStreak} sessions');
    if (state.courseStats.isNotEmpty) {
      sb.writeln('');
      sb.writeln('Per-Course Breakdown:');
      for (final c in state.courseStats) {
        sb.writeln(
            '  ${c.code} — ${c.attendanceRate.toInt()}%${c.isAtRisk ? " ⚠️ At Risk" : ""}');
      }
    }
    sb.writeln('─────────────────────────────');
    sb.write('Generated by AttendX');
        ...
        actions: [
          if (!state.isLoading)
            IconButton(
              ...
              onPressed: () => _shareAnalytics(context, state),
            ),
        ],
      ),
